...
export default function ClassBoardAssignment({ classId }) {
  ...
  useEffect(() => {
    axios.get(`/assignment-service/${classId}/assignment/all`)
      .then(res => {
  ...
  var classesList = filtered.map((clas) => {
    ...
    return (
      <tr>
        <th scope="row">{clas.assignmentId}</th>
        <td>
          <button onClick={enterassignmentdetail}>
            <a href={`/assignment-service/${classId}/assignment/${clas.assignmentId}`}>{clas.title}</a>
          </button>
        </td>
        <td>{clas.author}</td>
