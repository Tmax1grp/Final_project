...
export default function AdminClassItem({ item }) {
  ...
  const handleEditSubmit = () => {
    ...
    console.log(values);
    axios
      .put(`/admin-service/admin/classroom/${item.classId}`, null, {
        params: {
          name: values.name,
  ...
  useEffect(() => {
    setValues(item);
    if (item.status === 4) setStatusTxt("종료됨");
    else setStatusTxt("진행중");
  }, [item]);
    ...
    <>
      <tr>
        ...
        <td className={styles.MemberCell}>
          {item.createdDate !== null ? item.createdDate.split("T")[0] : ""}
        </td>
        <td className={styles.MemberCell}>
        ...
        <td className={styles.MemberCell}>
          <button className="clsbutton" onClick={showDeleteModal}>
            삭제
          </button>
        </td>
      </tr>
        ...
        <Modal.Body>
          ...
          <Form.Group as={Row} className="mb-3" controlId="participantNum">
            ...
            <Col sm="8">
              <Form.Control
                type="number"
                defaultValue={item.participantNum}
                onChange={handleChange}
              />
            </Col>
          </Form.Group>
