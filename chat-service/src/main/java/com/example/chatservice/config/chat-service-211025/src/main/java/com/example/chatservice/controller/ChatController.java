...
//    ORDER BY id;
    ...
    @ResponseBody
    public List<ChatEntity> readChat(@PathVariable Long classId, @RequestBody ChatEntity chatEntity) {

        final List<ChatEntity> chatall =
                chatRepository.findChatEntityByAndClassIdAndFromIdOrFromIdOrToId(
                        classId,
...
//    }
    ...
    public void createChat(@RequestBody ChatEntity chatEntity, @PathVariable long classId) { //, @RequestParam("*") MultipartFile multipartFile) {
//        public void createChat(@RequestBody ChatEntity chatEntity, @PathVariable long classId, @RequestParam("images") MultipartFile multipartFile) throws IOException{
...
//        }
        ChatEntity chat = new ChatEntity();
        chat.setClassId(classId);
        chat.setFromId(chatEntity.getFromId());
        chat.setToId(chatEntity.getToId());
        chat.setChatContent(chatEntity.getChatContent());
        chat.setChatDate(LocalDateTime.now());
